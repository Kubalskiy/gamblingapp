/**
 * Created by stasi on 24.12.2015.
 */
...
Counter.ScrollDirection = {Downwards: -1, Mixed: 0, Upwards: 1};
Counter.ScrollAnimation = {FixedTime: 0, FixedSpeed: 1, SlotMachine: 2};
...
Counter.prototype.finishLoading = function () {
    ...
    var i = 0;
    totalWidth = 0;
    if (this.prefixText != '')totalWidth += this.createTextElement(this.prefixText, 'prefix', 'counter_character_left');
    for (i = 0; i < this.digitsNumber; i++) {
        if (this.showDecimalSeparator && this.decimalSeparator && this.decimalPoints > 0 && i == this.digitsNumber - this.decimalPoints)totalWidth += this.createTextElement(this.decimalSeparator, 'decimal', 'counter_character_inner');
        if (this.showThousandsSeparator && this.thousandsSeparator && this.decimalPoints >= 0 && i > 0 && i < this.digitsNumber - this.decimalPoints && i % 3 == (this.digitsNumber - this.decimalPoints) % 3)totalWidth += this.createTextElement(this.thousandsSeparator, 'thousand_' + i, 'counter_character_inner');
        var temp = document.createElement("DIV");
        temp.id = this.wrapperId + "_" + i;
        temp.className = "counter_character";
        if (i == 0)temp.className += (this.prefixText != '' ? " counter_character_inner" : " counter_character_left") + (this.extraClassName.left ? " " : "") + this.extraClassName.left; else if (i == this.digitsNumber - 1)temp.className += (this.posfixText != '' ? " counter_character_inner" : " counter_character_right") + (this.extraClassName.right ? " " : "") + this.extraClassName.right; else temp.className += " counter_character_inner" + (this.extraClassName.inner ? " " : "") + this.extraClassName.inner;
    ...
    if (this.postfixText != '')totalWidth += this.createTextElement(this.postfixText, 'postfix', 'counter_character_right');
...
Counter.prototype.animate = function (forceFinish, callback) {
    if (forceFinish) {
...
Counter.prototype.add = function (addValue, duration, callback) {
    this.setValue(parseFloat(this.value) + addValue, duration, callback)
};
Counter.prototype.setValue = function (newValue, duration, callback) {
    ...
    for (i = this.digitsNumber - 1; i >= 0; i--) {
        ...
        if (this.direction == Counter.ScrollDirection.Mixed) {
            if (Math.abs(current - next) > this.characterNumber / 2) {
                if (next < current)this.beforeAnimation[i] = this.beforeAnimation[i] + this.digitHeight * this.characterNumber; else this.beforeAnimation[i] = this.beforeAnimation[i] - this.digitHeight * this.characterNumber
            }
    ...
    if (this.scrollAnimation == Counter.ScrollAnimation.FixedSpeed) {
        var maxDistance = 0.0;
        for (i = 0; i < this.beforeAnimation.length; i++)if (Math.abs(this.beforeAnimation[i] - this.afterAnimation[i]) > maxDistance)maxDistance = Math.abs(this.beforeAnimation[i] - this.afterAnimation[i]);
        for (i = 0; i < this.beforeAnimation.length; i++) {
    ...
    this.value = newValue;
    if (this.onValueChanged != null)this.onValueChanged();
    if (!(duration && parseInt(duration) > 0))duration = 1000;
    this.isAnimating = true;
    var stepDuration = this.animationDuration;
    this.animationStep = (stepDuration / duration);
    this.animationProgress = 0.0;
    var that = this;
    if (duration > stepDuration)this.intervalTimerId = setInterval(function () {
        that.animate(false, callback)
    }, stepDuration);
    this.timeoutTimerId = setTimeout(function () {
        that.animate(true, callback)
    }, duration)
